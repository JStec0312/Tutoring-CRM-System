# US-006 — Add student by invitation

> As a tutor, I want to add a student manually or by invitation so that I can start working with them in the system.

## Scope

A tutor invites a student by email. The student accepts the invitation (must be logged in with the invited email) and a `TutoringAgreement` is created automatically. Only the invitation flow is implemented; there is no "add student manually" endpoint.
